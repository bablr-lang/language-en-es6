  ...
  class ES6StringGrammar extends Base {
    *TemplateString({ getState }) {
      ...
      if (tagged) {
        ...
        yield eat(
          m`openToken*: <* '\u0060' { balanced: '\u0060', balancedSpan: 'String:TaggedTemplate' } />`,
        );
      } else {
        yield eat(m`tag+$: null`);
        yield eat(
          m`openToken*: <* '\u0060' { balanced: '\u0060', balancedSpan: 'String:Template' } />`,
        );
      }
    ...
    *StringInterpolation() {
      ...
      yield eat(m`openToken*: <* '}' { balancer: true } />`);
    }
