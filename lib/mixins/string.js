...
const mixin = (Base) => {
  class ES6StringGrammar extends Base {
    ...
    *StringContent({ s }) {
      ...
      do {
        ...
        esc =
          span.name === 'String:TaggedTemplate'
            ? yield eatMatch(m`<EscapeSequence '\\\u0060' />`, o({}), o({ escape: true }))
            : yield eatMatch(m`<EscapeSequence '\\' />`, o({}), o({ escape: true }));
      } while (esc || lit);
    }
    ...
    *EscapeSequence(args) {
      ...
      if (s.span.name === 'String:Template') {
        ...
        if ((_match = yield match(m`/[\\/bfnrt0\u0060]/`))) {
          const match_ = printSource(_match);
          yield eat(m`code*: <*Keyword ${printString(match_)} />`);
  ...
  freezeClass(ES6StringGrammar);

  return ES6StringGrammar;
};
