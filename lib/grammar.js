...
import { freeze, freezeClass, deepFreezeRecord } from '@bablr/agast-helpers/object';
...
class ES6Atrivial extends importMixin(classMixin(functionMixin(stringMixin(ES5.atrivial)))) {
  static context = deepFreezeRecord({
    ...super.context,
    assignables: [...arrayValues(super.context.assignables), 'CapturePattern'],
  });
        ...
        switch (chr) {
          ...
          default: {
            if (
              power >= 4 &&
              (res = yield eatMatch(
                (unaryMatcher ??= m`<UnaryExpression ${printSource(
                  buildPattern(unaryPrefixOperatorAlternatives),
                )} />`),
              ))
            ) {
            } else {
